...
interface DeviceSyncSidebarProps {
  ...
  /** Optional iPod collection actions shown below the main sync button. */
  showCollectionActions?: boolean;
  collectionSyncLabel?: string;
  collectionMirrorLabel?: string;
  onCollectionSyncSelected?: () => void;
  onCollectionMirror?: () => void;
  disableCollectionSync?: boolean;
  disableCollectionMirror?: boolean;
  syncLibraryOnlyFromCollection?: boolean;
  onSyncLibraryOnlyFromCollectionChange?: (value: boolean) => void;
...
export function DeviceSyncSidebar({
  ...
  collectionSyncLabel,
  collectionMirrorLabel,
  onCollectionSyncSelected,
  onCollectionMirror,
  disableCollectionSync = false,
  disableCollectionMirror = false,
  syncLibraryOnlyFromCollection = false,
  onSyncLibraryOnlyFromCollectionChange,
  mirrorDeleteFromDevice = false,
  ...
  const shouldShowCollectionSyncButton =
    showCollectionActions &&
    Boolean(onCollectionSyncSelected) &&
    Boolean(collectionSyncLabel) &&
    collectionSyncLabel !== syncButtonLabel;
  const shouldShowCollectionMirrorButton =
    showCollectionActions && Boolean(onCollectionMirror) && Boolean(collectionMirrorLabel);
  const shouldShowCollectionOptions = showCollectionActions;
    ...
    <aside className="w-full lg:w-80 lg:max-w-sm shrink-0 sticky top-0 self-start">
      <div className="rounded-lg bg-app-surface p-4 space-y-4">
        ...
        <div className="flex flex-col gap-2">
          {showScanButton &&
            devicePathDetection?.scanProgress != null &&
              ...
              <div className="rounded-md border border-app-border bg-app-surface p-3">
                <div className="flex items-center justify-between gap-2 mb-1.5">
                  ...
                  <span className="text-app-secondary text-xs tabular-nums">
                    {devicePathDetection.scanProgress.scanned} files
                  </span>
                </div>
                <div className="w-full bg-app-hover rounded-full h-2 overflow-hidden">
                  <div
                    className="h-2 rounded-full bg-accent-primary animate-scan-indeterminate"
                    style={{ width: "40%" }}
                  />
                </div>
                <div className="mt-1.5 text-app-tertiary text-xs">
                  Scanned {devicePathDetection.scanProgress.scanned}, matched{" "}
                  {devicePathDetection.scanProgress.matched}, hashed{" "}
                  {devicePathDetection.scanProgress.hashed}
                </div>
              </div>
            ) : (
              devicePathDetection.scanProgress.scanned > 0 && (
          ...
          {(shouldShowCollectionSyncButton ||
            shouldShowCollectionMirrorButton ||
            shouldShowCollectionOptions) && (
            <div className="space-y-2">
              {shouldShowCollectionSyncButton && (
                <Button
                  variant="primary"
                  onClick={onCollectionSyncSelected}
                  disabled={disableCollectionSync}
                  className="w-full justify-center"
                >
                  {collectionSyncLabel}
                </Button>
              ...
              {shouldShowCollectionMirrorButton && (
                <Button
                  variant="secondary"
                  onClick={onCollectionMirror}
                  disabled={disableCollectionMirror}
                  className="w-full justify-center"
                >
                  {collectionMirrorLabel}
                </Button>
                ...
                <div className="space-y-2 text-app-primary text-xs">
                  <label className="flex items-start gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        onSyncLibraryOnlyFromCollectionChange?.(e.target.checked)
                      }
                      className="rounded border-app-border mt-0.5"
                    />
                  ...
                  <label className="flex items-start gap-2">
                    <input
                      ...
                      onChange={(e) => onMirrorDeleteFromDeviceChange?.(e.target.checked)}
                      className="rounded border-app-border mt-0.5"
                    />
                  ...
                  <label className="flex items-start gap-2">
                    <input
                      ...
                      onChange={(e) =>
                        onOverwriteExistingPlaylistOnIpodChange?.(e.target.checked)
                      }
                      className="rounded border-app-border mt-0.5"
                    />
          ...
          {devicePreset === "ipod" && (
            <p className="pt-2 mt-2 border-t border-app-border text-app-tertiary text-xs">
              Syncing updates the iPod&apos;s track and playlist database with the current
              device content plus this sync. Artwork is only updated when you add new
              artwork; existing artwork is kept otherwise. A backup of the database is
              made before writing and removed after a successful sync.
            </p>
          )}
        </div>
