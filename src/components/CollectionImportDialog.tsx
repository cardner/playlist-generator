...
import { AlertCircle, CheckCircle2, X, Music, Calendar, HardDrive } from "lucide-react";
...
export function CollectionImportDialog({
  ...
  const handleConfirm = () => {
    // If creating new and there's an existing collection, require a name
    if (action === "create" && existingCollection && !newName.trim()) {
      return; // Can't create without a name when there's a conflict
    }
    onConfirm({
      ...
      <div className="bg-app-surface rounded-sm border border-app-border shadow-2xl max-w-2xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        <div className="p-6">
          ...
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-app-primary text-lg font-semibold">Import Collection</h2>
            <button
              onClick={onCancel}
                  ...
                  <div className="flex-1">
                    ...
                    {action === "create" && (
                      <input
                        ...
                        className="w-full px-2 py-1.5 bg-app-surface text-app-primary rounded-sm border border-accent-primary focus:outline-none focus:ring-1 focus:ring-accent-primary text-sm"
                        autoFocus
                      />
                    )}
                  </div>
